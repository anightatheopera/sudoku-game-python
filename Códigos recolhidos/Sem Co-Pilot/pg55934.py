...
def Check_Validity(board, row, col, num):
    block_x = math.ceil((row + 1) / 3) * 3 - 2
    block_y = math.ceil((col + 1) / 3) * 3 - 2
    ...
    for i in transpose_board[col]:
        if (i == num):
            if row == 0 and col == 0:
                print("false2")
            return False

    for i in range(block_x-1, block_x+2):
        for j in range(block_y-1, block_y+2):
            if (board[i][j] == num):
                if row == 0 and col == 0:
                    print("false3")
                return False
...
def Generate_Unsolved_Puzzle(board):
    ...
    for row in range(0, 9):
        chosen = []
        for i in (range(0, 5) if row == 0 else range(0, 4)):
